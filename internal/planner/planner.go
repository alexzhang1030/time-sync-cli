...
func renderPHC2Sys(iface string) string {
	return strings.TrimSpace(fmt.Sprintf(`
# Generated by timesync-cli
-f /etc/timesync-cli/ptp4l.conf -s %s -w -R 8 -N 5 -S 1.0
`, iface)) + "\n"
}

func renderPHC2SysMaster(iface string) string {
	// -O 37 keeps the PHC on the PTP/TAI time scale (system UTC + TAI–UTC).
	// Without it, phc2sys locks PHC to CLOCK_REALTIME and publish-gm-time-properties
	// never sees a TAI residual within tolerance, so currentUtcOffsetValid stays 0.
	return strings.TrimSpace(fmt.Sprintf(`
# Generated by timesync-cli
-s CLOCK_REALTIME -c %s -O 37 -w -S 1.0
`, iface)) + "\n"
}
...
ExecStart=/usr/sbin/phc2sys -f /etc/timesync-cli/ptp4l.conf -s %s -w -R 8 -N 5 -S 1.0
Restart=on-failure
...
[Service]
# Allow PHC step + GM property publication after start.
TimeoutStartSec=120
ExecStart=/usr/sbin/phc2sys -s CLOCK_REALTIME -c %s -O 37 -w -S 1.0
ExecStartPost=%s publish-gm-time-properties --timeout 60s
Restart=on-failure
